const parseDataset = (content) => {
  ...
  lines.forEach(line => {
    ...
    if (/\S/.test(line) && line.charAt(0) !== '%') {
      const trimmedLine = line.split('\r')[0].replace('\t', ' ')
      if (readingDataInstances) {
        const dataInstance = parseDataInstance(trimmedLine)
      ...
      } else if (trimmedLine.substring(0, 9).toUpperCase() === '@RELATION') {
        dataset.relation = parseRelation(trimmedLine)
        relationFound = true
      } else if (trimmedLine.substring(0, 10).toUpperCase() === '@ATTRIBUTE') {
        if (!relationFound) {
          throw new Error('Invalid .arff-file')
        }
        const attribute = parseAttribute(trimmedLine)
        dataset.headers ? dataset.headers.push(attribute) : dataset.headers = [attribute]
      } else if (trimmedLine.substring(0, 5).toUpperCase() === '@DATA') {
        if (!relationFound) {
          throw new Error('Invalid .arff-file')
        }
